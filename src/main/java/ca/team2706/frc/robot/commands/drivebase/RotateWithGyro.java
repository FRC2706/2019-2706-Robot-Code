...
public class RotateWithGyro extends DriveBaseCloseLoop implements IMirrorable<Command> {
    ...
    protected final Supplier<Double> angleSupplier;
     ...
     *                      ending the command.
     */
    public RotateWithGyro(double angle, int minDoneCycles) {
     ...
     *                                 the target position.
     */
    public RotateWithGyro(Supplier<Double> angle, Supplier<Integer> minCyclesWithinThreshold) {
    ...
    @Override
    public void execute() {
        DriveBase.getInstance().setRotation((mirrored ? -1 : 1) * angleSupplier.get());
    }
