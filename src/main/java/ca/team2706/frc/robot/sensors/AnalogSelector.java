...
public class AnalogSelector extends SendableBase {
    ...
    private static Range[] voltageRanges;
    private final AnalogInput analogInput;

	public static Range[] getVoltageRanges() {
		if(voltageRanges == null) {
voltageRanges = new Range[] {new Range(0, 2.5), new Range(2.5, 2.75),
...
return voltageRanges;
    ...
    public int getIndex() {
        ...
        for (int i = 0; i < getVoltageRanges().length; i++) {
            // Check if the voltage is within the current voltage range
            if (getVoltageRanges()[i].isWithin(voltage)) {
                index = i;
                break;
            }
        }
